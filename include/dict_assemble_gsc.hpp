...
>
class dict_assemble_gsc{
public:
    static std::string type()
    {
        return "dict_assemble_gsc-"+ std::to_string(t_method)+"-" +std::to_string(t_block_size)+"-"+ std::to_string(t_estimator_block_size);
    }
	...
	static void create(collection& col, bool rebuild,size_t size_in_bytes) {
		...
		if (! utils::file_exists(fname) || rebuild ) {  // construct
			...
			for(size_t i = 0; i < text.size()-t_estimator_block_size+1;i++) {				
				...
				if(i < beginPos + t_estimator_block_size-1) continue;
				else {
					if(block_counts.find(hash) != block_counts.end()) //assemble
						...
						if(found == true && (i - beginPos >= 32 && i - beginPos <= 1024)) {//write while removing duplicates
					    	...
					    	if(uniqueSegs.find(seg_hash) == uniqueSegs.end()) {
