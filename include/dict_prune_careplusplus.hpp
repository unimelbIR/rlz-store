...
// struct dict_prune_careplusplus {
//     struct segment_info {
//         uint64_t offset;
//         uint64_t length;
...
//     {
//         return "dict_pruned_careplusplus-" + std::to_string(t_freq_threshold) + "-" + std::to_string(t_length_threshold) + "-" + std::to_string(t_method);
//     }
...
//     static uint32_t compute_nfac(t_dict_idx& idx, const sdsl::int_vector_mapper<8, std::ios_base::in>& dict, uint64_t offset)
//     {
//         auto seg_start = dict.begin() + offset;
//         auto seg_end = seg_start + t_block_size;
...
//             ++factor_itr;
//         }
//         return factors_produced;
//     }

//     //pruning base on max coverage: 1. could be global; 2. could be iterative!
