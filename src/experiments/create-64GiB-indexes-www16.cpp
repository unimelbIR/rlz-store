...
*/ 
        ...
        auto rlz_store = rlz_type_zzz_greedy_sp_multibale_local_half_norm_rand<factorization_blocksize,false>::builder{}
                     .set_rebuild(args.rebuild)
                     .set_threads(args.threads)
                     .set_dict_size(dict_size_in_bytes)
                     .build_or_load(col);
        compare_indexes(col,rlz_store, "Local_half_norm_rand");
        LOG(INFO) << "Multibale_Local_half_norm_rand compression ratio (include dic) = "
                  << 100.0 * (double) rlz_store.size_in_bytes() / (double) rlz_store.text_size;
        LOG(INFO) << "Multibale_Local_half_norm_rand compression ratio (exclude dic) = "
                  << 100.0 * (double) (rlz_store.size_in_bytes() - dict_size_in_bytes)/ (double) rlz_store.text_size;
