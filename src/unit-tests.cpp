...
TEST(factor_coder, factor_coder_blocked_subdict_zzz)
{
    ...
    for(size_t i=0;i<n;i++) {
        ...
        p0_coder c;
        {
            c.encode_block(os,spep_vec,bfd);
        }
    }
    }
    std::mt19937 gen(4711);
    ...
    for(size_t i=0;i<n;i++) {
        ...
        {
            c.decode_block(is,spep_vec,0,bfd_recover,bfd.num_factors);
        }
...
TEST(factor_coder, factor_coder_blocked_subdict_pv)
{
    const uint32_t factorization_blocksize = 64 * 1024;
    const uint64_t dict_size_bytes = 256 * 1024 * 1024;
    const uint64_t literal_threshold = 3;
    ...
    using p0_coder = factor_coder_blocked_subdict_pv<literal_threshold,dict_page_size,num_pages_in_dict,dict_size_bytes,coder::elias_fano>; 
...
TEST(factor_coder, factor_coder_blocked_subdict_pv_ip)
{
    const uint32_t factorization_blocksize = 64 * 1024;
    const uint64_t dict_size_bytes = 256 * 1024 * 1024;
    const uint64_t literal_threshold = 3;
    ...
    using p0_coder = factor_coder_blocked_subdict_pv<literal_threshold,dict_page_size,num_pages_in_dict,dict_size_bytes,coder::interpolative>; 
